(function ($) {
  ...
  var DayScheduleSelector = function (el, options) {
    ...
    this.dates = generateDates(this.options.startTime, this.options.endTime, this.options.interval);
    this.render();
    this.attachEvents();
  ...
  DayScheduleSelector.prototype.renderHeader = function () {
    ...
    $.each(days, function (i, _) { 
        ...
        //html += '<th>' + stringDays[i] + ' ' + dates[0].getDate() + '/' + (dates[0].getMonth()+1) +'</th>'; 
        //dates[0].setDate(new Date(dates[0]).getDate() + 1);
      });
  ...
  function generateDates(start, end, interval) {
    ...
    var refDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate(), start.split(':')[0], start.split(':')[1]);
    while(refDate.getDay() != 0) refDate.setDate(new Date(refDate).getDate() + 1);
    return $.map(new Array(numOfRows), function (_, i) {
      // need a dummy date to utilize the Date object
      return new Date(refDate.getTime() + i * interval * 60000);
    });
  }
